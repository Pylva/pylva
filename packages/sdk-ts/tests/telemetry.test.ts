...
describe('telemetry HTTP exporter', () => {
  ...
  it('resumes telemetry after re-init following a 401', async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (!new URL(href).pathname.endsWith('/api/v1/events')) {
  ...
  it('retains a batch when a successful response has a malformed shape', async () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    let telemetryRequests = 0;
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      const path = new URL(input instanceof Request ? input.url : String(input)).pathname;
      if (!path.endsWith('/api/v1/events')) {
        return new Response(JSON.stringify(path.endsWith('/rules') ? { rules: [] } : { models: [] }), {
          status: 200,
        });
      }
      telemetryRequests += 1;
    ...
    expect(bufferSize()).toBe(1);
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining('malformed success response'),
    );
